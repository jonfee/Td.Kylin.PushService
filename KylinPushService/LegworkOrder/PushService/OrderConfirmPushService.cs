...
using Td.Kylin.Push.WebApi.JPushMessage.Legwork;
using Td.Kylin.Redis;

namespace KylinPushService.LegworkOrder.PushService
{
    public class OrderConfirmPushService : BaseLegworkService
    {
            ...
            while (true)
            {
                try
                {
                    ...
                    else if (apiConfig.Method == "post")
                    {
                        ...
                        loger.Success("用户确认订单，推送给工作端", "推送结果:" + postRst);
                    }
                }
                catch (Exception ex)
                {
                    if (error++ <= 5)
                    {
                        ...
                        loger.Write("用户确认订单，推送给工作端送时异常", ex);
                    }
                }
                error = 0;
            }
